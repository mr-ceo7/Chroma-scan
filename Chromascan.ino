...
#define LCD_CYCLE_DELAY 200 // min ms between display updates
...
// Clear display and show two lines of text (centered, size-1 font)
void oledPrint(const char *line1, const char *line2) {
  ...
  display.setCursor((SCREEN_WIDTH - len1 * 6) / 2, 20);
  display.print(line1);

  int len2 = strlen(line2);
  display.setCursor((SCREEN_WIDTH - len2 * 6) / 2, 40);
  display.print(line2);

  display.display();
}

// Overloads for Flash String Helper to save RAM
void oledPrint(const __FlashStringHelper *line1, const __FlashStringHelper *line2) {
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);

  int len1 = strlen_P((PGM_P)line1);
  display.setCursor((SCREEN_WIDTH - len1 * 6) / 2, 20);
  display.print(line1);

  int len2 = strlen_P((PGM_P)line2);
  display.setCursor((SCREEN_WIDTH - len2 * 6) / 2, 40);
  display.print(line2);
...
void oledPrint(const char *line1, const __FlashStringHelper *line2) {
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
...
// Print a single line at a given pixel row, clearing that row first
void oledPrintLine(int yPixel, const char *text) {
  display.fillRect(0, yPixel, SCREEN_WIDTH, 8, SSD1306_BLACK);
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(0, yPixel);
  display.print(text);
  display.display();
}

void oledPrintLine(int yPixel, const __FlashStringHelper *text) {
  display.fillRect(0, yPixel, SCREEN_WIDTH, 8, SSD1306_BLACK);
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(0, yPixel);
 ...
 * Shows animated progress on OLED during measurement.
...
void measureAllChannels(float *readings) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    char scanMsg[22];
    snprintf_P(scanMsg, sizeof(scanMsg), PSTR("Scan: %s"), labels[i]);
    oledPrintLine(40, scanMsg);

    // Animate dots while LED settles
    for (int d = 0; d < 3; d++) {
      char dotMsg[22];
      snprintf_P(dotMsg, sizeof(dotMsg), PSTR("Scan: %s %s"), labels[i], dots[d]);
      oledPrintLine(40, dotMsg);
      delay(200);
    }
    ...
    char resultMsg[22];
    snprintf_P(resultMsg, sizeof(resultMsg), PSTR("%s: %d OK"), labels[i], (int)readings[i]);
    oledPrintLine(40, resultMsg);
    delay(700);
...
void displayConcEntry() {
  ...
  display.print(F("Conc("));
  display.print(unitLabels[currentUnit]);
  display.print(F("):"));

  char buf[10];
  snprintf_P(buf, sizeof(buf), PSTR("%d%d%d.%d%d"), concDigits[0], concDigits[1],
  ...
  // Position the underline under the active digit
  ...
  display.print(F("Srt:+1  Lng:Next"));
...
void displayResultPage() {
  ...
  display.setTextSize(1);
  char pageInd[6];
  snprintf_P(pageInd, sizeof(pageInd), PSTR("[%d/5]"), resultPage + 1);
  ...
  case 0: // Red Absorbance
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.print(F("Abs(Red):"));
    display.setTextSize(2);
    display.setCursor(0, 20);
    display.print(absorbance[0], 4);
    break;

  case 1: // Green Absorbance
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.print(F("Abs(Green):"));
    display.setTextSize(2);
    display.setCursor(0, 20);
    display.print(absorbance[1], 4);
    break;

  case 2: // Blue Absorbance
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.print(F("Abs(Blue):"));
    display.setTextSize(2);
    display.setCursor(0, 20);
    display.print(absorbance[2], 4);
    break;
  ...
  case 3: // Best wavelength
    ...
    display.setTextSize(2);
    display.setCursor(0, 20);
    display.print(channelNames[bestChannel]);
    display.setTextSize(1);
    display.setCursor(0, 44);
    display.print(F("A="));
    display.print(absorbance[bestChannel], 4);
    break;

  case 4: // Concentration
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.print(F("Concentration:"));
    display.setTextSize(2);
    display.setCursor(0, 20);
    if (curveValid && concentration >= 0) {
      display.print(concentration, 2);
      display.setTextSize(1);
      display.setCursor(0, 44);
      display.print(unitLabels[currentUnit]);
    } else {
  ...
  display.print(F("Srt:Next Lng:NewSmpl"));
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDR)) {
    // If OLED fails, blink the buzzer LED as error indicator
    for (;;) {
      tone(BUZZER, 200, 200);
      delay(500);
    }
  ...
  // "Galvaniy" is 8 characters. Size 2 width = 8 * 12 = 96 pixels.
  display.setCursor((SCREEN_WIDTH - 96) / 2, 12);
  display.print(F("Galvaniy"));

  display.setTextSize(1);
  // "Technologies" is 12 characters. Size 1 width = 12 * 6 = 72 pixels.
  ...
  // Welcome screen 2 — product name + version
  display.clearDisplay();
  display.drawRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, SSD1306_WHITE);

  display.setTextSize(2);
  // "Chroma" is 6 characters. Size 2 width = 6 * 12 = 72 pixels.
  display.setCursor((SCREEN_WIDTH - 72) / 2, 10);
  display.print(F("Chroma"));

  // "Scan" is 4 characters. Size 2 width = 4 * 12 = 48 pixels.
  display.setCursor((SCREEN_WIDTH - 48) / 2, 30);
  display.print(F("Scan"));

  display.setTextSize(1);
  // "v1.0" is 4 characters. Size 1 width = 4 * 6 = 24 pixels.
  ...
  // Ready screen
  oledPrint(F("Press btn to"), F("begin..."));
...
void loop() {
  ...
  case STATE_WELCOME:
    if (btn == BTN_SHORT) {
      ...
      oledPrint(F("Settings"), F("Srt:Unit Lng:Skip"));
      beepSettings();
    }
    break;
  ...
  case STATE_SETTINGS:
    if (btn == BTN_SHORT) {
      ...
      display.setTextSize(1);
      display.setCursor(0, 50);
      display.print(F("Srt:Next Lng:OK"));
      display.display();
      beepSettings();
    } else if (btn == BTN_LONG) {
      currentState = STATE_CAL_BLANK;
      oledPrint(F("Insert BLANK"), F("Press to calibr."));
    }
    break;
  ...
  case STATE_CAL_BLANK:
    if (btn == BTN_SHORT) {
      ...
      oledPrint(F("Calibrating..."), F("Please wait..."));
      ...
      display.setTextColor(SSD1306_WHITE);
      display.setCursor(0, 0);
      display.print(F("Baseline saved!"));
      display.drawLine(0, 10, SCREEN_WIDTH, 10, SSD1306_WHITE);

      char buf[22];
      display.setCursor(0, 16);
      snprintf_P(buf, sizeof(buf), PSTR("R: %d"), (int)I0[0]);
      display.print(buf);
      display.setCursor(0, 28);
      snprintf_P(buf, sizeof(buf), PSTR("G: %d"), (int)I0[1]);
      display.print(buf);
      display.setCursor(0, 40);
      snprintf_P(buf, sizeof(buf), PSTR("B: %d"), (int)I0[2]);
      display.print(buf);
  ...
  case STATE_CAL_DONE:
    if (btn == BTN_SHORT) {
      ...
      char buf[22];
      snprintf_P(buf, sizeof(buf), PSTR("Standards: %d/%d"), numStandards,
               MAX_STANDARDS);
      oledPrint(buf, F("Srt:Add Lng:Done"));
    }
    break;
  ...
  case STATE_STD_MENU:
    if (btn == BTN_SHORT) {
      if (numStandards >= MAX_STANDARDS) {
        beepError();
        oledPrint(F("Max standards!"), F("Long press: Done"));
      } else {
        currentState = STATE_STD_MODE_SELECT;
        stdEntryMode = 0;
        oledPrint(F("Entry mode:"), F("Srt:Manual Lng:Pre"));
      }
    } else if (btn == BTN_LONG) {
      if (numStandards >= 2) {
        ...
        if (curveValid) {
          ...
          display.setTextColor(SSD1306_WHITE);
          display.setCursor(0, 10);
          display.print(F("Curve ready!"));
          display.setTextSize(2);
          display.setCursor(0, 28);
          display.print(F("m="));
          display.print(slopeM, 4);
          display.display();
          delay(1500);
        } else {
          oledPrint(F("Curve error!"), F("Check standards"));
          beepError();
          delay(1500);
        }
        ...
        oledPrint(F("Need min 2 stds"), F("Add more or skip"));
        beepError();
        delay(1500);
        break;
      }
      currentState = STATE_MEASURE_SAMPLE;
      oledPrint(F("Insert SAMPLE"), F("Press to measure"));
    }
    break;
  ...
  case STATE_SET_CONC_MANUAL:
    if (btn == BTN_SHORT) {
      ...
      if (concDigitPos >= 5) {
        ...
        display.setTextColor(SSD1306_WHITE);
        char buf[22];
        snprintf_P(buf, sizeof(buf), PSTR("Std #%d saved!"), numStandards);
        display.setCursor(0, 0);
        display.print(buf);
        display.drawLine(0, 10, SCREEN_WIDTH, 10, SSD1306_WHITE);
        display.setCursor(0, 20);
        display.print(F("A="));
        display.print(bestAbs, 4);
        display.setCursor(0, 32);
        display.print(F("Ch: "));
        display.print(channelNames[bestCh]);
        display.setCursor(0, 56);
        display.print(F("Press to continue"));
        display.display();
  ...
  case STATE_MEASURE_SAMPLE:
    if (btn == BTN_SHORT) {
      ...
      curveValid = false;
      currentState = STATE_CAL_BLANK;
      oledPrint(F("== RESET =="), F("Insert BLANK"));
      beepError();
      delay(1000);
      oledPrint(F("Insert BLANK"), F("Press to calibr."));
    }
